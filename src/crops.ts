...
export class Crop {
	...
	async endDrag() {
		...
		const prevLocalPosX =
			this.dragTarget.originalPosition.x - this.grid.gridInfo[0][0].x;
		const prevLocalPosY =
			this.dragTarget.originalPosition.y - this.grid.gridInfo[0][0].y;
		...
		if (
			localPosX < 0 ||
			localPosY < 0 ||
			localPosY > gridBorder ||
			localPosX > gridBorder
		)
			// out of grid
		...
		if (currIndex != targetIndex || targetIndex == -1)
			return returnToOriginal();
		...
		this.renderer.app.stage.off("pointermove", () => {
			this.moveDrag(this.dragTarget);
		});
	}
